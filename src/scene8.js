    ...
    create() {
        ...
        if (this.game.globalState.certified == false) {
            createMenu(this, {
                ...
                options: [
                    "[ plant illegally ]",
                    "[ ask to trade seeds with neighbors ]"],
                callbacks: [
                    () => {
                        this.scene.start("planttiming", { nextScene: "scene9", sourceScene: "scene8" });
                    },
                    () => {
                        this.game.globalState.neighborScore -= 1;
                        this.scene.get('hud').updateStats();
                        this.scene.start("slots", { nextScene: "scene9", sourceScene: "scene8" });
